  ...
  return (
    <span className="flex gap-0.5" aria-hidden>
      {Array.from({ length: 5 }, (_, i) => (
        <Star key={i} size={size} weight="fill" className="text-clay" />
      ))}
    </span>
  );
}
...
function ReviewCard({ review }: { review: Review }) {
  return (
    <figure className="w-[300px] shrink-0 rounded-2xl border border-latte bg-milk p-6 md:w-[360px]">
      <Stars />
      <blockquote className="mt-4 text-[15px] leading-relaxed text-espresso">
        &ldquo;{review.quote}&rdquo;
      </blockquote>
      <figcaption className="mt-4 text-sm">
        <span className="font-semibold">{review.name}</span>
        <span className="text-cocoa"> &middot; {review.source}</span>
      </figcaption>
    </figure>
  );
}
        ...
        <Reveal>
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 flex items-center gap-3 text-lg text-cocoa">
                <Stars size={20} />
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-sm font-semibold text-espresso underline-offset-4 hover:underline"
              >
                Read all reviews
